...
import math
import time
from pprint import pprint #pprint is pretty printing it used to displaying class information
class hero:
...
def enemyGen(levelBoss): 
    ...
    adjective = lines[random,random.randint(0,len(lines)-1)][:-1] #it gonna be a random intger to 0 and the ammount of  lines in the file -1 it reads  from one and all the way don to the end [:-1] it remove symbol at end
    file.close
    file = open ("animal.txt","r")
    lines = file.readlines()
    animal = lines[random,random.randint(0,len(lines)-1)][:-1]
    file.close
...
def getSuperMove(self):
        return self.superMove
def setSuperMove(self, newSuperMove):
        self.superMove = newSuperMove    
def createClass():
    a = input("Are you more strategic(1) or more of a Warrior(2)?... ")
    while a != "1" and a!= "2": # != not equal
        print("Invalid selection I dont know if you did that on purpose is not that hard to press 1 or 2")
        a = input("Are you more strategic(1) or more of a Warrior(2)?...")
    ...
    b = input("press enter to roll a dice... is 100 procent rng ;)....") #this goin to decide user luck 
    time.sleep(1.2)
    print ("rolling dice.... ooo what will you get") #is delay the program for only 0,2
    heroLuck = random.randint(0,10) #genret a number to 1 to 10
    print ("your hero has", heroLuck, "luck out of 10")

    c = input("are you more of a bow and arrow(1) user or magic user(2)? ")
    while c != "1" and c!= "2":
        print ("invalid slection Nah not agian you did it last time")
        c = input("are you more of a bow and arrow(1) user or magic user(2)?")
    ...
    heroName = input("What is your name mighty hero? ")
    print ("Welcome", heroName, "!!!")

    
    return (heroAttack, heroLuck, heroRanged, heroDefence, heroMagic, heroName) #we need to return virbal we createt in class (it has to be in order)


class_data = createClass() #this varibal gonna run createClass we did (scroll up u find ;3) at the same time the reutrn fuction we need for the hero stats going allso in the varibal. we can use Class_data as a list

character = hero(100, class_data[0], class_data[1], class_data[2], class_data[3], class_data[4], class_data[5] ); #we are calling the hero class to this varibal and why class_data [1] and so on becuase we did it in order in return and we known that heroAttack is class_data[1]as kai said you start in 0 not one thanks alot kai
